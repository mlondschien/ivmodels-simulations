...
output = FIGURES_PATH / "kleibergen19_clr_size"
input = DATA_PATH / "kleibergen19_clr_size"
...
def main(n, k, n_vars, lambda_max, n_seeds, cov_type):
    ...
    for idx, (ax, test_name) in enumerate(zip(axes.flat, tests)):
        ...
        # ax.set_xlim(lambda_1s.min(), lambda_1s.max())
        # ax.set_ylim(lambda_2s.min(), lambda_2s.max())
        # ax.set_xscale("log")
        # ax.set_yscale("log")
    ...
    plt.savefig(
        ...
        / f"figure_kleibergen19_{cov_type}_k{k}_2x2_tau05.pdf",
